...
La validación vigente pasa `npm run build`, `npm test` (104 tests TypeScript), `cargo test --manifest-path desktop/src-tauri/Cargo.toml` (18 tests Rust) y la generación del bundle `.app`. El arranque manual del bundle se ha ejecutado en macOS; el smoke gráfico automatizado queda pendiente.
