...
Las gates `build` y `tests` citan ejecuciones reales: una configuración de `run-configurations` declara `verifies: "build"` o `verifies: "tests"`, y al terminar bajo una Task escribe evidencia `verification.<qué>.pass` o `.fail` con su código de salida y la cola de su salida. La gate lee la evidencia más reciente de su tipo; un Project que nunca ejecutó una verificación la deja `pending`, y un run detenido por el operador no escribe evidencia porque no prueba nada. El contrato está en [ADR-0044](../adr/0044-verification-gates-from-real-runs.md).
