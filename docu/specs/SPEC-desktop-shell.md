...
- Un Assay sin Projects registrados no tiene Project abierto y lo dice: los tres controles de contexto —Project, Task y rama— muestran la misma marca `—`, y el panel invita a registrar una carpeta. Una fila donde cada control deletrea su vacío de forma distinta se lee como tres clases de vacío en vez de una. Qué Project abre al arrancar lo decide el catálogo —el que nombre el entorno si está registrado, si no el primero que haya—, nunca un identificador que la shell suponga. Un Project que ya no está registrado responde `PROJECT_NOT_FOUND` y vacía la shell en lugar de dejar el anterior en pantalla.
- Todas las superficies de trabajo comparten el mismo suelo, y cada tema dice cuál es: en oscuro es el propio color de ventana, que es lo que hace que la shell se lea como una sola superficie; en claro es el fondo de editor de Everest contra el tono del cromo, porque un IDE construido sobre esa paleta se reconoce por el escalón entre ambos y aplanarlo convierte la ventana entera en un lavado de un solo tono. El suelo es un token con nombre, no el color de fondo clavado.  `Projects`, `Agents`, `Editor` y `Version control` se dibujan sobre el color de fondo y se estructuran con líneas de 1px, no con tarjetas de un tono más claro. Lo que flota —menús, diálogos, campos y barras elevadas— conserva el tono elevado, porque algo que flota tiene que distinguirse de aquello sobre lo que flota, y el cromo —sidebar, dock de terminal y barra de estado— conserva el suyo.
- Las preferencias del operador —tono de fin de turno, modelo por defecto por proveedor y feed de releases— viven en la base de datos de ADE y se editan en un diálogo abierto desde la topbar, según [ADR-0053](../adr/0053-user-settings-live-in-ades-store.md). Lo que describe la ventana y no al operador —tema, alturas, anchos, columnas plegadas y pestañas abiertas— sigue en el almacenamiento local del webview. Lo que este webview ya recordaba se traslada una vez, de modo que actualizar no reinicia en silencio esas elecciones.
- Crear una Task pide intención y criterios de aceptación, uno por línea; sin al menos uno la Task no puede pasar a `READY` y la shell lo dice donde el operador todavía puede escribirlos. El detalle de la Task muestra esos criterios inmediatamente encima de `Approve` y `Ship` —aprobar es compararse con ellos— y permite cambiarlos, lo que queda registrado en el historial de la Task.
...
Dos popups de teclado complementan ese filtro sin vivir en la topbar ni en el Explorer: `Go to file` y `Recent files`, con su contrato completo en [SPEC-file-workspace](SPEC-file-workspace.md#product-contract). Ambos se cierran con Escape o con un click en el fondo atenuado del `<dialog>`, nunca con un click dentro de su propio contenido.
...
El Editor trata binarios, ficheros ilegibles y previews demasiado grandes con estados explicativos, salvo la imagen raster, el SVG y el HTML, que ganan preview propio ([ADR-0060](../adr/0060-image-and-html-preview.md)). `Open externally` es una acción separada y explícita; seleccionar un fichero nunca debe lanzar automáticamente una aplicación del sistema. La lectura se solicita al backend Tauri y queda sometida a la autorización de la raíz del Project. CodeMirror cubre la ruta principal inmediatamente; Monaco, sus contribuciones y Prettier se dividen e importan bajo demanda al abrir un lenguaje fallback o solicitar `Format`. El alcance completo está en [file-workspace](SPEC-file-workspace.md#product-contract).
...
El mensaje del agente conserva la traza del turno que lo produjo —proveedor y modelo, duración, actividad pública, ficheros tocados con sus líneas y consumo con coste cuando el proveedor lo declara— plegada bajo la respuesta y desplegable con puntero o teclado; la respuesta sigue siendo lo que la conversación lee. Es actividad verificable, no cadena de pensamiento privada, según [ADR-0047](../adr/0047-turn-trace-kept-with-the-answer.md). Un turno que termina emite un tono corto, tanto si se completó como si falló; no suena cuando el operador lo detuvo, porque estaba delante. Está encendido por defecto, se silencia desde la cabecera de la conversación y la preferencia es del operador, no del Project. El aviso remoto —correo o webhook— queda congelado con su diseño registrado en [ADR-0046](../adr/0046-turn-chime-and-frozen-remote-notice.md). Lo que el turno consumió se dice en su traza con la cache aparte de la entrada, la conversación acumula su total bajo los diales del rail y la Task lo suma en su detalle, según [ADR-0049](../adr/0049-turn-cost-read-where-the-work-is.md); una conversación o una Task que ningún turno contabilizó se lee como no reportada y jamás como cero. Esa lectura la hace la shell por su cuenta, igual que la comprobación de versión, así que un sidecar que no sepa responderla —una app todavía en marcha desde una instalación anterior— deja el dato en desconocido y no abre el diálogo de operación fallida, reservado para lo que el operador sí intentó.
...
La shell actual se verifica con `npm run build`, `npm test` (531 tests TypeScript), `cargo test --manifest-path desktop/src-tauri/Cargo.toml` (52 tests Rust), `npm run desktop:package:app` y smoke macOS; el smoke gráfico automatizado continúa pendiente. Esa misma secuencia se ejecuta por matriz en macOS, Windows y Linux según [SPEC-cross-platform-support](SPEC-cross-platform-support.md#verification-strategy); macOS es hoy la única plataforma verificada.
...
- El escape hatch mínimo abre la raíz del Project en Terminal mediante Tauri y permite configurar un comando externo de IDE; la integración profunda con un editor o IDE externo concreto queda fuera.
