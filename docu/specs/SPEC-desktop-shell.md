...
- Un Assay sin Projects registrados no tiene Project abierto y lo dice: la topbar muestra `No project`, la rama `—` y el panel invita a registrar una carpeta. Qué Project abre al arrancar lo decide el catálogo —el que nombre el entorno si está registrado, si no el primero que haya—, nunca un identificador que la shell suponga. Un Project que ya no está registrado responde `PROJECT_NOT_FOUND` y vacía la shell en lugar de dejar el anterior en pantalla.
