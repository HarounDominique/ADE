...
- Todas las superficies de trabajo comparten el mismo suelo: `Projects`, `Agents`, `Editor` y `Version control` se dibujan sobre el color de fondo y se estructuran con líneas de 1px, no con tarjetas de un tono más claro. Lo que flota —menús, diálogos, campos y barras elevadas— conserva el tono elevado, porque algo que flota tiene que distinguirse de aquello sobre lo que flota, y el cromo —sidebar, dock de terminal y barra de estado— conserva el suyo.
