...
ADE debe instalarse y ejecutarse en macOS, Windows y Linux sin que el usuario note qué plataforma fue la primera. El objetivo no es abstraer el sistema operativo, sino aislar los pocos puntos donde ADE lo toca y verificar cada uno en la plataforma real, de forma que una regresión de plataforma falle en CI y no en la máquina de alguien.
...
ADE toca el sistema operativo en cinco sitios, y sólo en esos cinco. Cualquier código nuevo que necesite un sexto es una señal de que la frontera se está filtrando.
...
Tampoco entra abstraer Git: ADE seguirá invocando el `git` del sistema y exigiendo que esté en `PATH`.
