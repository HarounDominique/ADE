...
**Corrección posterior (2026-09-12):** una auditoría de paridad Windows pedida por el operador —no un bug reportado— encontró dos defectos más, invisibles desde macOS por la misma razón que los anteriores. El árbol del Explorer comparaba rutas pendientes con `===` crudo entre una ruta construida a mano con `/` y la ruta nativa que Rust entrega en `dataset.directoryPath`; ahora usa el mismo helper de comparación por segmentos que ya exige la frontera 7. Y `validate_new_entry_name` —compartida por New File, New Directory y Rename— no rechazaba los nombres reservados de Windows (`CON`, `PRN`, `AUX`, `NUL`, `COM1`-`COM9`, `LPT1`-`LPT9`, sin distinguir mayúsculas y reservados incluso con extensión), los caracteres que Windows prohíbe en un nombre (`< > : " | ? *` y caracteres de control) ni un punto final; la validación se aplica ahora en las tres plataformas a la vez, no sólo detrás de `cfg(windows)`, porque el objetivo es que un repositorio creado en macOS no contenga nunca un nombre que sólo rompe al abrirse en Windows. Ninguno de los dos cambia el grado `no verificada` de abajo: siguen sin una corrida real de la matriz de Windows que los confirme.
...
Empaquetado firmado y notarizado, instaladores adicionales de Linux (AppImage u otros paquetes de distribución), actualización automática —descarga y reemplazo del bundle por la propia aplicación—, soporte de arquitecturas distintas de x86-64 y ARM64 donde el runner no las ofrezca, y paridad visual pixel a pixel entre sistemas.
...
9. ✅ Ubuntu produce un paquete `.deb` con un comando (`npm run desktop:package`), el smoke manual del 2026-09-10 cubre el recorrido operativo completo y el tag `v<version>` tiene workflow para publicarlo.
10. ✅ Un tag `v<version>` construye en runners nativos y publica en una única release los artefactos Ubuntu, macOS y Windows junto con un `latest.json` fusionado. Sin cerrar todavía: la firma de artefactos, una instalación Ubuntu real vía `dpkg -i`/`apt install` — el smoke se hizo sobre el binario extraído del `.deb`, no instalado — y la duplicación de `libwebkit2gtk-4.1-0`/`libgtk-3-0` en `Depends`.
