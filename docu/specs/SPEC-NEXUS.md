...
El dock inferior expone tabs de terminal PTY persistentes, redimensionables y confinados a la raíz del Project. Cada tab conserva su proceso, transcript, cwd, historial y completado; cambiar de tab no mezcla salida y cerrar uno detiene sólo su sesión. El prompt mínimo, Enter, historial con `↑`/`↓`, completado de directorios para `cd` con `Tab` e interpretación ANSI de cursor, borrado y pantalla alternativa están implementados; las sugerencias ambiguas se recorren con `↑`/`↓` y se cierran con `Esc`. `Version control` muestra historial de commits, ficheros modificados y diffs; su pestaña `Changes` lista pendientes y permite `Commit & Push` o `Fetch origin` con confirmación. La baseline de verificación actual es 96 tests TypeScript y 18 tests Rust. El `.app` macOS se empaqueta y se ha arrancado manualmente; el smoke gráfico automatizado sigue pendiente. `Projects` permite añadir y retirar del seguimiento carpetas Git o No Git sin tocar sus ficheros, y la selección de ficheros abre ahora dentro de ADE en `Editor`, definido en [SPEC-file-workspace](SPEC-file-workspace.md#product-contract), con edición, guardado y descarte acotados al Project.
