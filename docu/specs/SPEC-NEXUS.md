...
El dock inferior expone tabs de terminal `portable-pty` persistentes, redimensionables y confinados a la raíz del Project. Cada tab conserva su proceso y superficie `xterm.js`; cambiar de tab no mezcla salida y cerrar uno detiene sólo su sesión. `Terminal.onData` reenvía directamente cada secuencia al PTY y `Terminal.onResize` sincroniza filas y columnas. El shell real conserva prompt, historial y completado, mientras xterm.js interpreta ANSI, cursor, borrado, color y pantalla alternativa, por lo que los prompts interactivos de Claude siguen siendo navegables sin campo HTML ni heurísticas de ADE. `Version control` muestra historial de commits, ficheros modificados y diffs; su pestaña `Changes` sigue el flujo de GitHub Desktop, con lista de ficheros seleccionables, diff por fichero con líneas resaltadas y un botón `Commit` en la cabecera que abre el diálogo modal de título y cuerpo. `Commit` sólo crea el commit local; `Push origin` permanece como acción posterior independiente y `Fetch origin` como acción independiente con confirmación. El panel operativo `Git workspace` solo se muestra dentro de `Version control`; la topbar conserva el contexto global de Project y branch. La baseline de verificación actual es 112 tests TypeScript y 18 tests Rust. El `.app` macOS se empaqueta y se ha arrancado manualmente; el smoke gráfico automatizado sigue pendiente. `Projects` permite añadir y retirar del seguimiento carpetas Git o No Git sin tocar sus ficheros, y la selección de ficheros abre ahora dentro de ADE en `Editor`, definido en [SPEC-file-workspace](SPEC-file-workspace.md#product-contract), con edición, guardado y descarte acotados al Project.
