...
- Una pestaña se puede arrastrar **fuera de la barra de pestañas** para separarla: el gesto abre exactamente la misma ventana que `New window`. Salir de la barra —al editor, a otro panel, al escritorio o a otro monitor— significa lo mismo: ese fichero quiere ventana. Mientras se arrastra, una etiqueta con el nombre sigue al cursor y se marca cuando soltar haría algo; la barra ya no es el único sitio donde el gesto es visible. El gesto se sigue con eventos de ratón, no con el arrastre propio del webview ni con eventos de puntero: macOS enruta el ratón a la ventana que recibió la pulsación hasta soltarlo, así que la soltada llega aunque el cursor haya salido de la ventana, que es justo el dato que hace falta. Cualquier pestaña se puede levantar; si puede irse o no se responde al soltarla —una que aún se está leyendo, o que no es texto, lo dice—, porque una pestaña que se niega a ser levantada no enseña nada. Sigue sin existir un evento de «soltado fuera»: la posición de la soltada se compara con el rectángulo de la propia barra, con un margen que evita que una arrastrada horizontal descuidada cuente como salir. Lo que sigue al cursor lo dibuja Assay, porque la imagen de arrastre nativa venía con un enfoque que no funcionaba y se fue con él. Si esos límites no se pueden leer, no se separa nada: perder una pestaña por accidente es peor que un gesto que no hace nada.
...
- Mientras el filtro tiene texto, el árbol pertenece a la búsqueda: ninguna carga del árbol pinta encima de unos resultados vivos. La única excepción es cambiar de Project, que además limpia la caja, porque un filtro con texto sobre un árbol que ya no describe es otra forma de mentir. La regla vive en la carga, no en cada uno de los sitios que la invocan.
- El filtro del Explorer busca en los ficheros que el repositorio dice que son del operador: lo trackeado más lo no trackeado que ninguna regla ignora, en una sola llamada a Git. No hay que adivinar que este proyecto compila en `target` y el siguiente en `build`: las reglas ya están escritas en el repositorio. La consecuencia declarada es que un fichero ignorado no aparece en la búsqueda —el árbol sí lo sigue mostrando—, y es el intercambio correcto: una búsqueda que devuelve artefactos de compilación no es una búsqueda. Sin Git, la búsqueda recae en el recorrido a mano, que salta `.git` y `node_modules` —almacenamiento de Git y árbol de dependencias, donde nadie edita—, ordena las coincidencias de nombre antes que las de ruta y las someras antes que las profundas, y se detiene en 200. Una lista cortada lo dice donde termina, porque acabar en silencio se leería como «no hay nada más», que es otra respuesta.
...
- Buscar (`Mod-f`), siguiente coincidencia (`F3`/`Mod-g`) y anterior (`Shift-F3`/`Shift-Mod-g`) funcionan igual en CodeMirror y Monaco sin código adicional de ADE: ambos motores ya los traen por defecto. Reemplazar (`Mod-r`) es el único atajo que hizo falta añadir, uno por motor, porque Monaco reserva `Mod-r` para otra acción por defecto.
- `Go to file` (`Ctrl+Shift+N` en Windows/Linux, `Cmd+Shift+O` en macOS, cualquiera de los dos aceptado en cualquier plataforma) abre un popup que filtra ficheros en vivo sobre el mismo `search_directory` que usa el filtro del Explorer, con navegación por flechas, apertura con Enter o click y cierre con Escape. `Recent files` (`Ctrl+E`/`Cmd+E`) lista hasta los 20 últimos ficheros abiertos en la sesión activa, más reciente primero, reutilizando la fila y el guardia de apilado del primer popup. Ninguno de los dos vive en la topbar ni reintroduce una segunda búsqueda de ficheros: son atajos de teclado sobre el mismo `openFileInADE` que ya usa el Explorer. Un click fuera de su contenido —el fondo atenuado del `<dialog>`— los cierra igual que Escape; un click dentro de su propio contenido nunca lo hace.
