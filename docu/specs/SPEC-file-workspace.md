...
- Una pestaña de texto se puede arrastrar fuera de la ventana para separarla: el gesto abre exactamente la misma ventana que `New window`. No existe un evento de «soltado fuera», así que se deduce en `dragend` —nadie dentro aceptó el drop— comparando el cursor con los límites de la ventana, con la escala del monitor aplicada. Si esos límites no se pueden leer, no se separa nada: perder una pestaña por accidente es peor que un gesto que no hace nada.
