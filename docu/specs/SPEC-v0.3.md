...
- The `Version control` view is the only surface that renders `Git workspace`: it reports the active branch and changed files as well as branches, worktrees and remotes. The Task-scoped UI exposes branch/worktree/commit/push/PR operations confirmed through the shell's own dialog, with worktree branch and path entered as real form fields; the topbar keeps global Project, Task and branch selectors.
...
El mantenimiento posterior del 2026-09-06 cerró la segunda pasada de `Version control` alineada con GitHub Desktop: `Changes` usa dos paneles reales con diff dominante; `History` permite contraer y restaurar independientemente commits y ficheros con controles anclados a sus cabeceras; y los diffs se adaptan al ancho disponible, incluyendo líneas largas. Estas mejoras no reabren v0.3 ni alteran el contrato Git local → Push separado. Una pasada posterior alineó el control de contracción de `Changed files` con el de `History`, hizo que `Push origin` dependiera de los commits que el remoto no ha visto en lugar de los creados en la sesión, y sustituyó los prompts nativos del navegador —inertes en el webview— por diálogos propios de la shell. Los cortes del 2026-09-07 unificaron Tasks dentro de `Projects`, reordenaron la navegación, hicieron visible el turno del agente desde el envío y ligaron el modelo elegido a su conversación; los del 2026-09-08 añadieron respuesta incremental y actividad pública verificable de los tres providers, además del historial de prompts por conversación. La verificación del corte anterior fue `npm test` con 129 tests TypeScript, `cargo test` con 19 tests Rust y bundle `.app` macOS; el smoke gráfico automatizado y `.dmg` continúan fuera de alcance.

La verificación vigente del comportamiento posterior a la release es `npm test` con 246 tests TypeScript y `cargo test` con 20 tests Rust. Las cifras anteriores se preservan como evidencia de los cortes que las produjeron; no son la baseline actual.

Tras cerrar v0.3, la shell recibió refinamientos de interacción que forman parte del comportamiento actual de la aplicación: selector persistente claro/oscuro en la esquina superior derecha, barra superior de contexto con cambio de Project, Task y branch local, Task limitada a 12 registros por creación descendente, identidad activa mantenida en un único estado frente a refreshes asíncronos, transición fluida y respeto de `prefers-reduced-motion` al cambiar el Explorer, hover legible en ambos temas y dock de terminal inferior redimensionable con un chevron que alterna sus alturas mínima y máxima.

La terminal actual presenta tabs de sesiones `portable-pty` independientes que ejecutan el `$SHELL` del usuario, sin bienvenida ni eco duplicados. Cada tab conserva su proceso y superficie `xterm.js`; `Terminal.onData` reenvía directamente al PTY todas las secuencias de teclado y `Terminal.onResize` sincroniza sus filas y columnas. El shell real conserva historial y completado, y xterm.js interpreta ANSI, cursor, borrado, color y pantalla alternativa; por ello Claude y otras TUIs reciben flechas, espacio, Enter, Escape, Tab, borrado, caracteres imprimibles y controles `Ctrl` sin heurísticas ni modo especial de ADE. La slice posterior v0.4 abre ficheros de texto dentro de ADE, permite edición, guardado y descarte, mantiene la apertura externa como acción explícita y presenta un Editor fijo y exclusivo para código, sin paneles auxiliares ni CTA de búsqueda duplicada; su contrato está en [SPEC-file-workspace](SPEC-file-workspace.md). Los refinamientos recientes del Explorer añaden búsqueda file-first con rutas contextuales, índice reutilizable, debounce y spinner transitorio. Estas mejoras no reabren la release ni amplían su alcance: editor completo, language server, autocompletado general de comandos, `.dmg`, cloud y colaboración realtime siguen fuera.
