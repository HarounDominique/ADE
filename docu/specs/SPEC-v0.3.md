...
La terminal actual presenta tabs de sesiones PTY independientes con prompt mínimo, sin bienvenida ni eco duplicados. Cada tab conserva su proceso, transcript, cwd, historial y completado; `Enter` ejecuta, `↑`/`↓` navegan el historial y `Tab` completa rutas de directorio en comandos `cd`; las coincidencias ambiguas se muestran como sugerencias navegables y `Esc` las oculta. El transcript interpreta ANSI para conservar el cursor, el borrado y la pantalla alternativa de TUIs interactivas como Claude. Mientras Claude u otra TUI ocupa la pantalla alternativa, o desde que se lanza un comando interactivo conocido, las teclas de navegación, espacio, Enter, Escape, Tab, borrado, caracteres imprimibles y controles `Ctrl` se reenvían al PTY sin ser consumidas por el historial/completado de ADE; pulsar cualquier parte del panel recupera el foco y al salir se restaura el modo shell. La slice posterior v0.4 abre ficheros de texto dentro de ADE, permite edición, guardado y descarte, y mantiene la apertura externa como acción explícita; su contrato está en [SPEC-file-workspace](SPEC-file-workspace.md). Los refinamientos recientes del Explorer añaden búsqueda file-first con rutas contextuales, índice reutilizable, debounce y spinner transitorio. Estas mejoras no reabren la release ni amplían su alcance: editor completo, language server, autocompletado general de comandos, `.dmg`, cloud y colaboración realtime siguen fuera.
