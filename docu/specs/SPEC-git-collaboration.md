...
Las acciones visibles `git.fetch.origin` y `git.commit.push` requieren `actor`, `reason` y `confirmed: true`. `git.commit.push` crea el commit con título y cuerpo opcional y después hace push de la rama actual a `origin`; cada fase informa de su resultado y un fallo de push no revierte un commit ya creado.
