...
Cada fila del working tree de `Changes` sustituye el código de estado Git crudo por un glifo coloreado —verde `+` para no trackeados/nuevos, rojo `−` para borrados, ámbar `º` para modificados, renombrados o en conflicto—, colapsando la misma clasificación que ya usa el árbol del Explorer en tres cubos propios de esta lista, sin tocar los colores de decoración del Explorer. Cada fila añade además una casilla, marcada por defecto, para que el operador decida qué ficheros entran en el siguiente commit; un alternador de selección total/nula vive junto al recuento de la cabecera. `git.commit.create` acepta un conjunto opcional de ficheros y, cuando llega, stagea exactamente ese conjunto de forma determinista —`git reset` seguido de `git add -- <ficheros>`, ignorando lo que ya estuviera stageado fuera de Assay—; sin ficheros marcados, `Commit` se niega con un mensaje en vez de crear un commit vacío. Omitir el parámetro conserva el `git add --all` de siempre. `Fetch origin` se deshabilita cuando el Project activo no declara remoto, igual que ya hacía `Push origin`, en vez de fallar con el error crudo de Git al pulsarlo.
...
Cuando el Project activo no tiene Git, el desplegable de rama sustituye su antiguo botón deshabilitado por `Initialize Git repository`: confirmada en el mismo diálogo `GIT OPERATION` que el resto de mutaciones sensibles, ejecuta `git init --initial-branch=master` en la raíz del Project —siempre `master`, con independencia del `init.defaultBranch` global del operador— y refresca topbar, `Version control` y el resto de la shell a su estado consciente de Git sin recargar la vista. El mismo desplegable abre, con `+ New branch`, un formulario inline con el nombre de la rama; confirmarlo ejecuta `git.branch.create` (`git switch -c <name>`) y refresca rama, lista del desplegable y `Version control` una vez la mutación ha terminado de verdad. `git init` reutiliza `assertConfirmed` y `executeGit`, igual que el resto de mutaciones Git del puerto.

Un `.git` borrado fuera de Assay mientras el Project sigue activo —Finder, terminal, otra herramienta— no debe propagar el `fatal: not a git repository` crudo de Git como un fallo modal inesperado, ni siquiera desde el sondeo de fondo de `git.pending`: cada lectura Git comprueba primero que el repositorio sigue existiendo y se recupera con un estado explícito en vez de un error genérico.

La topbar sustituyó su antiguo selector `Quick Open` de repositorios por `Current project`/`Current task`/`Current branch`; el filtro del Explorer sigue siendo la búsqueda de ficheros del árbol lateral. Esto no compite con el popup `Go to file` de apertura rápida de fichero por teclado, que no vive en la topbar y se documenta en [SPEC-file-workspace](SPEC-file-workspace.md#product-contract).

## Version control read model

El sidecar expone `git.history`, `git.commit.diff`, `git.pending` y `git.pending.diff` como lecturas acotadas al `repositoryPath` del Project activo. `git.history` devuelve commits, sus ficheros y si el remoto los ha visto, `git.commit.diff` devuelve el parche de un commit completo o de un fichero concreto, `git.pending` devuelve ficheros con estado y diff local incluyendo no trackeados en la lista, y `git.pending.diff` devuelve el parche del fichero pendiente seleccionado. Las lecturas no modifican el árbol. Se repiten cuando llega la atención del operador —al entrar en `Version control`, al abrir una de sus tabs y al volver a la aplicación con esa vista delante— porque un commit se hace casi siempre en otro sitio: la terminal del dock, otra ventana, otra herramienta. Nada encuesta a Git con un temporizador: costaría un par de procesos por tick durante todas las horas que la ventana esté abierta para que casi siempre no haya cambiado nada. Los momentos que se solapan se funden en una sola lectura, y `Refresh` sigue existiendo para forzarla.
