...
- Las propuestas de arranque de Spring Boot requieren una entry point verificable —`mainClass` configurada o una clase de aplicación con `main`—; un POM agregador (`packaging=pom` o con `modules`) sólo ofrece build/test. Un wrapper sin bit ejecutable se invoca mediante `sh` en POSIX para que un checkout o ZIP no convierta una propuesta válida en un error de permisos.
...
- **Early exit:** si el proceso termina después de `spawn` pero antes de mantenerse en ejecución, la sesión pasa a `FAILED` con código/señal y el diagnóstico queda visible; un miembro que termina hace fallar el compound y detiene los demás.
