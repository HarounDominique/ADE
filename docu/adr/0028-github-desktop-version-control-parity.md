...
Una pasada posterior del 2026-09-06 corrige tres desviaciones de esta paridad detectadas en uso. El control de contracción de `Changed files` se había quedado alineado a la izquierda por un `display: grid` sin columnas que anulaba el `flex` previsto; vuelve a comportarse como el de `History`, con el mismo icono, posición y animación. `Push origin` dependía de si el commit se había creado en la sesión en curso, de modo que un repositorio con commits pendientes mostraba el botón deshabilitado; ahora `git.history` marca cada commit con `unpushed` resolviéndolo contra el repositorio y la habilitación deriva de ahí, además de señalar esos commits en `History` y componer el estado remoto con las dos deudas. Las confirmaciones redundantes de `Push origin` y `Fetch origin` se retiran: su intención ya es explícita. `npm test` pasa con 120 tests TypeScript y `cargo test` con 18 Rust. La superficie de confirmación pasa a estar definida en [ADR-0030](0030-in-app-confirmation-surface.md).
