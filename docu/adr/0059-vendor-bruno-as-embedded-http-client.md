...
**Corrección de alcance (Phase 2):** de esta lista, Assay termina consumiendo directamente sólo `@usebruno/lang` (vía `@usebruno/filestore`) y `@usebruno/filestore`. `@usebruno/requests@0.20.0` se instaló y se descartó en la misma fase: su superficie real (verificada, no asumida — `dist/esm/index.d.ts`) es interceptores OAuth2/Digest/EdgeGrid, cliente gRPC, cliente WebSocket, resolución de proxy PAC y caché de agentes HTTP/HTTPS — ninguno de los cuales usa el contrato `HttpAuth`/`HttpBody` de [SPEC-http-client.md](../specs/SPEC-http-client.md#request-and-collection-contract) (`none`/`basic`/`bearer`/`apikey`; sin OAuth2 ni Digest; HTTP/HTTPS request-response, sin gRPC ni WebSocket, ambos fuera de alcance explícito de la spec). Instalarlo sólo añadía superficie sin uso —y con ella, transitivamente, las dos vulnerabilidades altas de `axios@1.16.0` y `@faker-js/faker@9.9.0` que motivaron el commit `fix: pin axios/faker above known-vulnerable versions`, luego revertido junto con la propia dependencia. Assay usa `axios` directamente (pinned `^1.20.0`, fuera de rango vulnerable) para la ejecución HTTP; `@usebruno/js` (sandbox de scripts) tampoco se adopta en esta fase porque `HttpAssertion` es comparación declarativa, no scripting. Ambos quedan disponibles para una fase futura si el alcance crece a scripting pre/post-request o a auth OAuth2/Digest — hoy no están instalados.
...
- **Corrección de Phase 2, resuelta por exclusión, no por override:** `@usebruno/requests@0.20.0` se instaló, se auditó (`npm audit`: altas en `axios@1.16.0` y `@faker-js/faker@9.9.0`, transitivas suyas) y se desinstaló en la misma fase al confirmar que su superficie real no cubre nada que el contrato `HttpAuth`/`HttpBody` de esta spec necesite — ver la corrección de alcance en la tabla de arriba. `axios@^1.20.0` queda como dependencia directa de Assay, fuera del rango vulnerable, sin necesidad de `overrides`: la vulnerabilidad desapareció con la dependencia que la traía, no con un pin sobre ella. `npm audit`: 0 hallazgos.
