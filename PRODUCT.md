...
- `Projects` reúne el catálogo local, el Project activo y sus Tasks en una sola vista maestro-detalle; `Tasks` no es una entrada de navegación independiente.
...
- El turno del agente es visible desde que se envía: el prompt entra en la conversación al pulsar Enter y bajo él un turno en vuelo informa de su estado, del tiempo transcurrido y de la actividad que el proveedor reporta en vivo. El modelo elegido pertenece a la conversación y se persiste con ella; cada agente puede tener un modelo por defecto, marcable desde el menú `Model`, que sólo decide en qué modelo arranca una conversación nueva y nunca reescribe la elección de una conversación existente. Cada mensaje persistido se puede copiar.
- `Agents` como workbench inline: rail de conversaciones del Project activo agrupadas por Task/General, contraíble con transición y conversación central dominante; provider y modelo dependiente viven en la cabecera contextual, mientras la Task vive en la topbar y permisos/envío junto al composer. El composer recupera con `↑`/`↓` los prompts de usuario guardados en la conversación activa, envía con `Enter` (`Shift+Enter` añade línea) y detiene el turno activo con `Esc`, abortando el runtime del proveedor. El inspector lateral de actividad, ficheros y skills no forma parte de la vista primaria.
- Workflow de desarrollo adaptativo que conduce la Task por ocho fases con cuatro modos, cuenta los intentos de cada fase, sube de tier en el segundo y para en el tercero para pedir decisión humana. Un guard determinista sobre el diff staged exige test junto a todo fichero de producción añadido o modificado y evidencia real de ejecución verde; `RECONCILE` deja reglas reutilizables validadas y acotadas. Viene activado y se apaga por operador o por Project, ganando el Project cuando se pronuncia; apagado conservan su comportamiento Tasks, gates, evidencia, review y aprobación.
