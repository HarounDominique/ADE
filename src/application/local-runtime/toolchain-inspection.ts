...
export async function inspectProjectToolchains(projectRoot: string): Promise<readonly ToolchainStatus[]> {
  ...
  for (const directory of directories) statuses.push(...await inspectDirectory(projectRoot, directory));
...
async function inspectDirectory(projectRoot: string, directory: string): Promise<readonly ToolchainStatus[]> {
  const statuses: ToolchainStatus[] = [];
  ...
  if (await exists(join(directory, "package.json"))) statuses.push(await probe(scopedId("node"), "Node/npm", "npm", ["--version"], source("package.json"), directory));
  if (await exists(join(directory, "pyproject.toml"))) {
    ...
    statuses.push(await probe(scopedId(python.id), python.label, python.command, python.args, source("pyproject.toml"), directory));
  }
  if (await exists(join(directory, "pom.xml"))) statuses.push(await probe(scopedId("maven"), "Maven", "mvn", ["--version"], source("pom.xml"), directory));
  if (await exists(join(directory, "build.gradle")) || await exists(join(directory, "build.gradle.kts"))) statuses.push(await probe(scopedId("gradle"), "Gradle", "gradle", ["--version"], source(await exists(join(directory, "build.gradle")) ? "build.gradle" : "build.gradle.kts"), directory));
  if (await exists(join(directory, "Cargo.toml"))) statuses.push(await probe(scopedId("rust"), "Rust/Cargo", "cargo", ["--version"], source("Cargo.toml"), directory));
  if (await exists(join(directory, "go.mod"))) statuses.push(await probe(scopedId("go"), "Go", "go", ["version"], source("go.mod"), directory));
  const entries = await readdir(directory, { withFileTypes: true }).catch(() => []);
  const dotnetManifest = entries.find((entry) => entry.isFile() && /\.(sln|csproj)$/.test(entry.name));
  if (dotnetManifest) statuses.push(await probe(scopedId("dotnet"), ".NET", "dotnet", ["--version"], source(dotnetManifest.name), directory));
  return statuses;
}
