...
test("provider registry reports unavailable providers without credentials", async () => {
  const providers = await inspectProviders({ opencodeUrl: "http://127.0.0.1:1", codexCommand: "command-that-does-not-exist" });
  assert.deepEqual(providers.map((provider) => provider.id), ["opencode", "codex"]);
