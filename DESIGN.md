...
The desktop shell is a two-column evidence workspace: one labeled navigation/sidebar and a flexible workbench. The sidebar combines view labels with a persistent project explorer, avoiding duplicate navigation surfaces. A visible vertical grip lets the user resize it by drag or keyboard, within bounds persisted per Project. A restrained top bar anchors Project, Task and branch context; Task sits between Project and branch, follows the same menu pattern, lists at most twelve most recently created Tasks, and retains an older active Task when necessary. `Agents` uses a Project-scoped conversation rail grouped by Task and General plus a wide conversation-first central thread; the entire rail can smoothly collapse into a persistent restore control when the user needs maximum reading width. Its compact header combines identity, provider, model and state without repeating global Task or Project context. The composer is reserved for writing, per-turn permissions and sending, with a vertically resizable textarea bounded to the live window; `Enter` sends, `Shift+Enter` keeps a line break, `↑`/`↓` traverse the active conversation's persisted user prompts and `Esc` stops the active turn. Tool activity and changed files stay attached to the relevant turn instead of becoming a permanent inspector. `Version control` uses the same reading rhythm: a focused change list, readable diff and deliberate commit action. Changes gives the diff the dominant width; History can collapse its commit and changed-file columns, with restore controls anchored to their headers, and long diff lines wrap to the live available width. Persistent surfaces use generous whitespace, quiet rules and tonal layering rather than a grid of unrelated cards. The native PTY dock remains available at the bottom and can be resized by drag or keyboard.
...
The operational `Git workspace` panel belongs exclusively to `Version control`; Projects, Editor, Agents, Work and Project context do not repeat Git status or actions. The top bar retains only the global Project, Task and branch selectors.
...
- **Behavior:** The native PTY dock is always available above the status bar. Its top grip resizes the panel by pointer; focused resizing also supports Arrow keys, Shift+Arrow larger steps, Home and End. A centered, labeled chevron directly beneath that intact grip alternates between the existing minimum and maximum height with a short ease-out transition, rotating to state the next action. Fine pointer and keyboard resizing remain immediate; reduced-motion preferences skip spatial movement. The chosen height is persisted for the active Project.
